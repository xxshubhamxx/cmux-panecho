...
/// Clock.now is synchronous; the lock protects only the clock/continuation seam.
final class RefreshTestClock: Clock, @unchecked Sendable {
    ...
    func advance(by duration: Duration, wakeSleepers: Bool = true) {
        lock.lock()
        currentInstant = currentInstant.advanced(by: duration)
        guard wakeSleepers else { lock.unlock(); return }
        var due: [Sleeper] = []
