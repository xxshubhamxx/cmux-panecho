#!/usr/bin/env python3
"""Generate the RC (release-candidate) app icon by recoloring the Debug icon.

Takes the AppIcon-Debug icons (which have an orange "DEV" banner) and:
1. Recolors the orange banner to green
2. Replaces the "DEV" text with "RC"

This preserves the exact same icon design, glow effects, and chevron
positioning as the debug icon, matching how generate_nightly_icon.py produces
the nightly variant.
"""
import os
import sys
...
# Target: green (distinct from nightly's purple and debug's orange)
GREEN = (40, 170, 95)
...
def recolor_banner(img: Image.Image) -> Image.Image:
    """Recolor the orange banner to green and replace DEV with RC."""
    img = img.convert("RGBA")
    w, h = img.size
    pixels = img.load()

    # Pass 1: Recolor orange pixels to green.
    # The debug icon's banner is (255, 107, 0) with anti-aliased edges.
    # We detect "orange-ish" pixels and remap them to green, preserving
    ...
    for y in range(h):
        for x in range(w):
            ...
            if r > 180 and g < 180 and b < 100 and r > g and r - b > 100:
                ...
                # Remap to green, preserving the intensity
                nr = int(GREEN[0] * orange_strength)
                ng = int(GREEN[1] * orange_strength)
                nb = int(GREEN[2] * orange_strength)
                pixels[x, y] = (nr, ng, nb, a)

    # Pass 2: Replace the "DEV" text with "RC".
    # First, blank out the existing text by filling the text region with
    # the banner color, then draw "RC" centered.
    #
    ...
    # (the DEV text is white on orange, now white on green)
    text_pixels = []
    ...
    if text_pixels:
        # Erase old text by painting over with the banner green
        ...
        draw.rectangle([min_x, min_y, max_x, max_y], fill=(*GREEN, 255))

        # Now draw "RC" centered in the banner
        text = "RC"
