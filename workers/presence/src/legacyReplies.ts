...
export interface StoredLegacyPhoneReply {
  replyId: string;
  macDeviceId: string;
  workspaceId: string;
  surfaceId: string;
...
export const LEGACY_PHONE_REPLY_TTL_MS = 15 * 60 * 1000;
...
export const MAX_LEGACY_PHONE_REPLY_TEXT_CHARS = 8_192;
export const MAX_LEGACY_PHONE_REPLY_ID_CHARS = 64;
export const MAX_LEGACY_PHONE_REPLY_TARGET_ID_CHARS = 128;
/** Bound for the enqueue request body read. */
export const MAX_LEGACY_PHONE_REPLY_BODY_BYTES = 64 * 1024;
...
export const LEGACY_PHONE_REPLY_NUDGE_REVISION = 1;

const REPLY_PREFIX = "phonereply:legacy:";

/** The subset of `DurableObjectStorage` the inbox uses (Map-backed in tests). */
export interface LegacyPhoneReplyStorage {
...
export type ParseLegacyPhoneReplyResult =
  | { ok: true; reply: Omit<StoredLegacyPhoneReply, "createdAtMs" | "expiresAtMs"> }
 ...
 * has nothing to type. */
export function parseLegacyPhoneReply(body: Record<string, unknown>): ParseLegacyPhoneReplyResult {
  const replyId = boundedId(body.replyId, MAX_LEGACY_PHONE_REPLY_ID_CHARS);
  if (!replyId) return { ok: false, error: "invalid_reply_id" };
  const macDeviceId = boundedId(body.macDeviceId, MAX_LEGACY_PHONE_REPLY_TARGET_ID_CHARS);
  if (!macDeviceId) return { ok: false, error: "invalid_mac_device_id" };
  const surfaceId = boundedId(body.surfaceId, MAX_LEGACY_PHONE_REPLY_TARGET_ID_CHARS);
  ...
  const workspaceId = body.workspaceId == null
    ? ""
    : boundedId(body.workspaceId, MAX_LEGACY_PHONE_REPLY_TARGET_ID_CHARS);
  if (workspaceId === null) return { ok: false, error: "invalid_workspace_id" };
  const notificationId = body.notificationId == null
    ? ""
    : boundedId(body.notificationId, MAX_LEGACY_PHONE_REPLY_TARGET_ID_CHARS);
  ...
  if (text.length > MAX_LEGACY_PHONE_REPLY_TEXT_CHARS) return { ok: false, error: "text_too_long" };
  return {
    ok: true,
    reply: {
      replyId,
...
async function loadAll(storage: LegacyPhoneReplyStorage): Promise<Map<string, StoredLegacyPhoneReply>> {
  return storage.list<StoredLegacyPhoneReply>({ prefix: REPLY_PREFIX });
}
...
async function pruneExpired(
  ...
  nowMs: number,
): Promise<StoredLegacyPhoneReply[]> {
  const all = await loadAll(storage);
  const live: StoredLegacyPhoneReply[] = [];
...
export type EnqueueLegacyPhoneReplyResult =
  | {
      ok: true;
      duplicate: boolean;
      pending: number;
...
export async function enqueueLegacyPhoneReply(
  storage: LegacyPhoneReplyStorage,
  reply: Omit<StoredLegacyPhoneReply, "createdAtMs" | "expiresAtMs">,
  nowMs: number,
): Promise<EnqueueLegacyPhoneReplyResult> {
  ...
  const stored: StoredLegacyPhoneReply = {
    ...reply,
    createdAtMs: nowMs,
    expiresAtMs: nowMs + LEGACY_PHONE_REPLY_TTL_MS,
  };
...
export async function ackPhoneReplies(
  ...
  for (const replyId of replyIds) {
    const bounded = boundedId(replyId, MAX_LEGACY_PHONE_REPLY_ID_CHARS);
    if (!bounded) continue;
...
export function parseLegacyPhoneReplyAck(body: Record<string, unknown>): ParseAckResult {
  ...
  for (const value of body.replyIds) {
    const bounded = boundedId(value, MAX_LEGACY_PHONE_REPLY_ID_CHARS);
