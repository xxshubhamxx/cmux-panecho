// The Apple logo glyph, shared by the Mac download button and the iOS
// TestFlight button. 814:1000 aspect; width derived from height so it fills.
export function AppleMark({ size = 19 }: { size?: number }) {
  const height = size;
  const width = (height * 814) / 1000;
  return (
    <svg
      width={width}
      height={height}
