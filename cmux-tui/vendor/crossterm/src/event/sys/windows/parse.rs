...
fn handle_surrogate(surrogate_buffer: &mut Option<u16>, new_surrogate: u16) -> Option<char> {
    match *surrogate_buffer {
        Some(buffered_surrogate) => {
            *surrogate_buffer = None;
            std::char::decode_utf16([buffered_surrogate, new_surrogate]).next().unwrap().ok()
        }
        None => {
...
fn get_char_for_key(key_event: &KeyEventRecord) -> Option<char> {
    ...
    let desired_case =
        if is_shift_pressed ^ is_capslock_on { CharCase::UpperCase } else { CharCase::LowerCase };
    ...
    if is_alt_code {
        ...
        match utf16 {
            ...
            unicode_scalar_value => {
                ...
                let kind =
                    if key_event.key_down { KeyEventKind::Press } else { KeyEventKind::Release };
