...
mod tests {
    ...
    fn set_fg_bg_add_attr() {
        let style = ContentStyle::new().with(Color::Blue).on(Color::Red).attribute(Attribute::Bold);
        ...
        styled_content =
            styled_content.with(Color::Green).on(Color::Magenta).attribute(Attribute::NoItalic);
