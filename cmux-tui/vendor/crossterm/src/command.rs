...
fn write_command_ansi<C: Command>(
    ...
    let mut adapter = Adapter { inner: io, res: Ok(()) };

    command.write_ansi(&mut adapter).map_err(|fmt::Error| match adapter.res {
        Ok(()) => panic!("<{}>::write_ansi incorrectly errored", std::any::type_name::<C>()),
        Err(e) => e,
    })
}
