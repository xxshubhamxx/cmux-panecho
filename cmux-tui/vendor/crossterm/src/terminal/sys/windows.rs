...
pub(crate) fn set_size(width: u16, height: u16) -> std::io::Result<()> {
    ...
    if current_size.width < window.left + width {
        if window.left >= i16::MAX - width {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "terminal width too large"));
        }
    ...
    if current_size.height < window.top + height {
        if window.top >= i16::MAX - height {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "terminal height too large"));
        }
...
mod tests {
    ...
    fn test_scroll_down_winapi() {
        let current_window = ScreenBuffer::current().unwrap().info().unwrap().terminal_window();

        scroll_down(2).unwrap();

        let new_window = ScreenBuffer::current().unwrap().info().unwrap().terminal_window();
