...
fn match_event(event: Event) {
    if let Some(key) = event.as_key_press_event() {
        match key {
            ...
            KeyEvent { modifiers: KeyModifiers::ALT, code, .. } => {
                println!("Alt + {:?}", code);
            }
...
fn main() {
    match_event(Event::Key(KeyEvent::new(KeyCode::Char('z'), KeyModifiers::CONTROL)));
    match_event(Event::Key(KeyEvent::new(KeyCode::Left, KeyModifiers::SHIFT)));
    match_event(Event::Key(KeyEvent::new(KeyCode::Delete, KeyModifiers::ALT)));
    match_event(Event::Key(KeyEvent::new(KeyCode::Right, KeyModifiers::ALT | KeyModifiers::SHIFT)));
